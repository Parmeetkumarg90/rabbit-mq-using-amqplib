...
export default class ProcessorService {
    ...
    private readonly maxRequeueCount = Number(process.env?.EVENT_MAX_REQUEUE_COUNT ?? 3);
    private readonly maxRetryCount = Number(process.env?.EVENT_MAX_RETRY_COUNT ?? 5);
    ...
    constructor(private readonly inboxRepo: InboxRepository, private readonly rabbitmqService: RabbitMqService) { }
    ...
    async consumeMessages() {
        try {
            ...
            this.consumerChannel.consume(this.eventQueue, async (message) => {
                if (message !== null) {
                    ...
                    for (let handlerName in this.messageHandlerNameMap) {
                        if (!requeueOptions[handlerName] || requeueOptions[handlerName] <= this.maxRequeueCount) {
                            const isAlreadyHandled = await this.eventAlreadyAcknowledgement(refineMessage.eventId, handlerName);
                            if (isAlreadyHandled) {
                                console.log(handlerName, " already handled");
                                continue;
                            }
                            if (this.messageHandlerNameMap[handlerName] && typeof this.messageHandlerNameMap[handlerName] === "function") {
                                let isSuccess = false;
                                for (let tryCount = 0; tryCount < this.maxRetryCount; tryCount++) {
                                    isSuccess = await this.messageHandlerNameMap[handlerName](refineMessage);
                                    if (isSuccess) {
                                        break;
                                    }
                                ...
                                if (!isSuccess) {
                                    await this.requeueAndAddCount(headers, refineMessage, handlerName);
                                }
                            }
                            else {
                            ...
                            if (this.consumerChannel) {
                                console.log("Message sent to dead letter queue with consumerTag: ",
                                    message.fields.consumerTag, "and deliveryTag: ",
    ...
    async eventAlreadyAcknowledgement(eventId: number, handler: string) {
        const isAlreadyGoneThrough = await this.inboxRepo.getInboxEventUsingEventIdAndHandlerName(eventId, handler);
    ...
    async handleMail(refineMessage) {
        try {
            ...
            await this.inboxRepo.addNewInboxEvent(refineMessage.eventId, 'handleMail');
            // mail logic here
    ...
    async handleNotification(refineMessage) {
        try {
            // console.log("Message consumed: ", refineMessage);
            await this.inboxRepo.addNewInboxEvent(refineMessage.eventId, 'handleNotification');
    ...
    async requeueAndAddCount(headers, refineMessage, HANDLER_NAME: string) {
        setTimeout(() => {
            const retryHeaders = { ...headers, requeueOptions: { ...headers?.requeueOptions || {}, } };
            retryHeaders.requeueOptions[HANDLER_NAME] = (headers?.requeueOptions?.[HANDLER_NAME] ?? 0) + 1;
            if (this.consumerChannel) {
                this.consumerChannel.publish(this.eventExchange,
                    'to-event-queue',
                    Buffer.from(JSON.stringify(refineMessage)),
                    { persistent: true, headers: retryHeaders }
                );
            }
            console.log("Message requeue count: ", retryHeaders.requeueOptions[HANDLER_NAME]);
